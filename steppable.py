...
for particle in particle.list:
    #I guess it will depend on how the data is brought into python
    # Linear spring btwn ij
    linearspring = hoomd.md.bond.Harmonic()
    linearspring.params["ij"] = dict(k= k_fiber, r0=0.0)
    integrator.forces.append(linearspring) #this adds the linear potential to the list

    # Linear spring ik

    # Angular springs, for every triplet labelled "bend" returns the angular potential
    bend = hoomd.md.angle.Harmonic()

    bend.params["bend"] = dict(
        k=K_bend,
        t0=theta_0
    )
